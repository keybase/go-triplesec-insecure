...
func TestVectorV3(t *testing.T) {
	...
	c, err := NewCipher([]byte("42"), nil, Version(3), functionThatPrintsUglyWarnings, isProduction)
	if err != nil {
		t.Error(err)
	}
...
func TestVectorV4(t *testing.T) {
	...
	c, err := NewCipher(key, nil, Version(4), functionThatPrintsUglyWarnings, isProduction)
	if err != nil {
		t.Error(err)
	}
	buf, err := c.Decrypt(ciphertext)
...
func TestSpec(t *testing.T) {
	for _, version := range []Version{3, 4} {
		...
		for _, v := range vs.Vectors {
			...
			fixedRandomnessCipher, _ := NewCipherWithRng(key, nil, version, NewRandomTapeGenerator(r), functionThatPrintsUglyWarnings, isProduction)
